...
from keras.layers import concatenate, Conv2D, Conv2DTranspose, Dropout, ReLU, BatchNormalization, Activation, Add, Multiply
...
def get_unet(minimum_kernel=8, do=0, activation=ReLU, iteration=1):
    ...
    # TODO: use K.stop_gradient(x) -> to tf.stop_gradient()
    cls_in = Lambda(lambda x: tf.stop_gradient(x), output_shape=lambda s: s)(cls_in)
...
# use on train.py
class Generator():  
    def __init__(self, batch_size, repeat, dataset, index):
        self.lock = threading.Lock()
        self.dataset = dataset
        with self.lock:
            self.list_images_all = prepare_dataset.getTrainingData(0, self.dataset, index)
            self.list_gt_all = prepare_dataset.getTrainingData(1, self.dataset, index)
            self.list_gt_all_onehot = prepare_dataset.getTrainingData(1, self.dataset, index, need_one_hot=True)
...
# NOTE: Add experiment data generator function for preds
class PredGenerator():
